...
public class DoorPanelQueue {
	...
	void putPrioriryDoorPanelRequest(DoorCommand doorCommand) {
		
		if (!doorPanelRequest.contains(doorCommand)) {
			System.out.println("Prioriry request put in queue");
			doorPanelRequest.offerFirst(doorCommand);
		}
	}
		...
		public void run() {
			while (true) {
				...
				while (doorPanelRequest.size() != 0) {
					System.out.println("Processing request " + doorPanelRequest.peek());
